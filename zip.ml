...
(* pleclech modify library to use Extc haxe library *)
  ...
  try
    ...
    | Deflated ->
        ...
        let out_pos = ref 0 in
					Extc.zlib_uncompress ~header:false
            (fun buf ->
        ...
        let crc = Extc.update_crc Int32.zero res 0 (String.length res) in
        if crc <> e.crc then
    ...
    | Deflated ->
        ...
        let crc = ref Int32.zero in
          Extc.zlib_uncompress ~header:false
            ...
            (fun buf len ->
              output oc buf 0 len;
              crc := Extc.update_crc !crc buf 0 len);
        if !crc <> e.crc then
          raise (Error(ifile.if_filename, e.filename, "CRC mismatch"))
  with End_of_file ->
    raise (Error(ifile.if_filename, e.filename, "truncated data"))
...
let add_entry data ofile ?(extra = "") ?(comment = "") 
  ...
  let crc = Extc.update_crc Int32.zero data 0 (String.length data) in
  let compr_size =
    match level with
      0 ->
    ...
    | _ ->
        let in_pos = ref 0 in
        let out_pos = ref 0 in
          Extc.zlib_compress ~level ~header:false
            (fun buf ->
      ...
      0 ->
        ...
        let rec copy sz =
          ...
          if r = 0 then sz else begin
            crc := Extc.update_crc !crc buf 0 r;
...
let add_entry_generator ofile ?(extra = "") ?(comment = "")
  ...
  | _ ->
      let (send, flush) = Extc.zlib_compress_direct ~level ~header:false
          (fun buf n ->
            output ofile.of_channel buf 0 n;
      ...
      (fun buf pos len ->
        check ();
          send buf pos len;
          uncompr_size := !uncompr_size + len;
          crc := Extc.update_crc !crc buf pos len
      ),
      (fun () ->
        check ();
          flush ();
